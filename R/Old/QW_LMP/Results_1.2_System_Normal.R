...
# How many intervals have a low LMP
full_data %>% 
  group_by(settlementdate) %>% 
  summarise(no_low_lmp = all(lmp>(-500))) %>% 
  summarise(sum(no_low_lmp)/n())



# 37% have atleast one lmp< -500

# REGRESSION
##################################
reg_coeffs <- full_data %>% 
  group_by(settlementdate) %>% 
  filter(all(lmp>(-500))) %>% ungroup() %>% 
...
reg_coeffs %>% 
  ...
  geom_histogram()+
  ggsave("Output/Regressions/New Results/QW_LMP/beta_systemnormal.png", width = 10)


full_data %>% 
  group_by(settlementdate) %>% 
  filter(all(lmp>(-500))) %>% ungroup() %>% 
  filter(duid %in% c("NBHWF1", "BASTYAN", "GSTONE1", "MCKAY1")) %>% 
  filter(abs(qw_lmp)<1000, abs(lmp)<1000) %>% 
